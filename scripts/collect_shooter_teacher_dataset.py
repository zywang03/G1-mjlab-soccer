"""Collect Stage II teacher rollouts for shooter student BC.

The script runs a trained Stage II recurrent shooter policy in the Eval-Shooter
environment, records motion-free student observations paired with teacher
actions, and labels each episode with valid-kick and goal-plane metrics.

Example:
  python scripts/collect_shooter_teacher_dataset.py \
      --checkpoint logs/rsl_rl/g1_soccer/<run>/model_20000.pt \
      --num-episodes 1024 --num-envs 64
"""
...
class CollectConfig:
  checkpoint: str
  """Stage II teacher checkpoint path."""
...
def _expected_cross_x(initial_ball: torch.Tensor, destination: torch.Tensor) -> torch.Tensor:
  ...
  alpha = (GOAL_Y - initial_ball[:, 1]) / (dy + torch.where(dy >= 0, 1e-6, -1e-6))
...
def _collect_batch(env, policy, cfg: CollectConfig) -> dict[str, Any]:
  ...
  fallback_length = torch.minimum(active_steps, torch.full_like(active_steps, seq_len))
  kick_length = torch.clamp(valid_kick_step + cfg.post_kick_steps + 1, min=0, max=seq_len)
  has_valid_kick = valid_kick_step >= 0
  keep_length = torch.where(has_valid_kick, torch.minimum(kick_length, fallback_length), fallback_length)
  time_ids = torch.arange(seq_len, device=device).unsqueeze(0)
  valid_mask = valid_mask & (time_ids.cpu() < keep_length.detach().cpu().unsqueeze(-1))
  ...
  metadata = {
    ...
    "kick_speed_step": kick_speed_step.cpu(),
...
def run_collection(cfg: CollectConfig) -> None:
  ...
  try:
    ...
    while remaining > 0:
      ...
      remaining -= take
      print(
        f"[INFO] shard={shard_idx:04d} episodes={int(total_summary['episodes'])}/{cfg.num_episodes} "
        f"success={int(total_summary['success'])} "
        f"rate={total_summary['success'] / max(total_summary['episodes'], 1):.3f}"
      )
  finally:
    env.close()
