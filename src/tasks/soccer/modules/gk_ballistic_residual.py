...
This module keeps a distilled goalkeeper base policy frozen and adds a small
residual head conditioned on compact ballistic features from ball history.
Checkpoints produced by the keeper-lyk repair pipeline store both ``base.*`` and
``residual.*`` weights, so eval can load them without an external base file.
"""
