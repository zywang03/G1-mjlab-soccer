...
\texttt{phase2/compete.py} sends the same raw state to both servers regardless of their role---each server extracts its own robot's data to compute observations.
...
\begin{itemize}
    ...
    The tournament is conducted centrally on a server provided by the teaching team. Each team deploys their policy servers (which must be network-reachable from the tournament server) and submits the access URLs. The teaching team runs \texttt{phase2/compete.py} or the \texttt{phase2/tournament\_server.py} web console for every pair of teams (Team A and Team B), evaluating both directions:
    \begin{itemize}
...
For Phase 2 cross-evaluation, teams deploy their policies as API servers and use \texttt{phase2/compete.py} or the web console in \texttt{phase2/tournament\_server.py} to run matchups:
...
The script loads two G1 robots into a single MuJoCo scene, reads raw MuJoCo state (joint angles, root poses, ball position/velocity), and sends the same state dict to both teams' API servers. Each team's server computes its own observation tensor and returns an action. Each episode lasts 5 seconds. This design fully decouples observation spaces across teams. A reference API server with example observation computation is provided as \texttt{phase2/api\_server.py}.
