...
Because each team will design unique observation spaces, feature extractors, and neural network architectures, your codebases will be heterogeneous. To enable cross-evaluation without requiring teams to share code or checkpoints, the tournament adopts a standardized API interface: each team deploys their trained policy behind a lightweight server and submits the server URLs to the teaching team's web console. The web console then launches the match executor, which loads two G1 robots into a single MuJoCo scene and queries each team's server at every simulation step.

\subsubsection{API Protocol}

All teams must implement a policy server exposing the following two endpoints (a reference implementation is provided as \texttt{phase2/api\_server.py}):
...
The tournament server sends the same raw state to both servers regardless of their role---each server extracts its own robot's data to compute observations.

\subsubsection{Timeline and Milestones}
\begin{itemize}
    \item \textbf{Weeks 13--16: Agent Development \& Improvement} \\
    Teams first complete their basic single-agent shooting and goalkeeping policies (Phase 1). They then continuously improve their agents' capabilities through adversarial training, self-play, or further single-agent optimization. And this phase will continue until 6.25.

    \item \textbf{Week 17: Report Submission \& Cross Evaluation} \\
    The final report and all required submission materials are due by June 26. By the same deadline, each team must deploy its policy servers (which must be network-reachable from the tournament server) and submit the access URLs. Cross evaluations will begin on June 26 and will be conducted centrally on a server provided by the teaching team. The teaching team uses the \texttt{phase2/tournament\_server.py} web console to start and monitor matchups. For each pair of teams (Team A and Team B), both directions are evaluated:
    \begin{itemize}
        \item Team A's Shooter vs.\ Team B's Goalkeeper (10 trials)
        \item Team B's Shooter vs.\ Team A's Goalkeeper (10 trials)
    \end{itemize}
    The specific evaluation schedule will be announced before June 26.
\end{itemize}
...
For Phase 2 cross-evaluation, teams deploy their policies as API servers and submit their API URLs through the teaching team's web console. The direct match executor \texttt{phase2/compete.py} is mainly a teaching-side implementation detail and debugging entrypoint; the expected workflow for students is to keep their API servers running and provide URLs that are reachable from the tournament server.

\begin{verbatim}
# Start policy servers (one per robot)
python phase2/api_server.py --checkpoint <path> --port 8000 --task shooter
python phase2/api_server.py --checkpoint <path> --port 8001 --task goalkeeper
\end{verbatim}

In the web console, the teaching team enters the shooter team name, shooter API URL, goalkeeper team name, goalkeeper API URL, and number of trials. The console displays a Viser URL for each running match; after opening that page, the match begins only after clicking \texttt{Start Trials}. The server then loads two G1 robots into a single MuJoCo scene, reads raw MuJoCo state (joint angles, root poses, ball position/velocity), and sends the same state dict to both teams' API servers. Each team's server computes its own observation tensor and returns an action. Each episode lasts 5 seconds. This design fully decouples observation spaces across teams. A reference API server with example observation computation is provided as \texttt{phase2/api\_server.py}. More detailed student-side instructions are provided in \texttt{phase2/README\_CLIENT.md}.
